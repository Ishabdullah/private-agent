...
- **Vosk-based custom-phrase fallback (Tier 2b)**: ~~meaningfully heavier...~~ **Moot as of Phase 13's audit (2026-08-25): superseded entirely.** The Phase 3/5b decision log ("Wake-word engine: Vosk-only, no Porcupine" then the Section 8.3.2 engine-change note) means Vosk was replaced by sherpa-onnx before any code shipped — there is no Vosk fallback anywhere in the app, and no tier-2b duty-cycling to build.
...
- **RAM**: ~~keeping a second (background) Flutter engine resident...~~ **Moot as of Phase 13's audit (2026-08-25): the alternative was adopted outright, not just prototyped.** `VoiceAssistantForegroundService.kt` runs the whole wake-word loop (AudioRecord + sherpa-onnx KeywordSpotter) in pure Kotlin with no Dart engine involved; Dart/Flutter only spins up when `notifyWakeWordDetected()` launches `MainActivity` for an actual command. There is no second background Flutter engine anywhere in the shipped architecture (the one other Flutter-engine-adjacent feature, the floating overlay window, is unrelated to this pipeline and is currently disabled via `FeatureFlags.floatingOverlayEnabled`).
- **Wake-word model reload cost checked (2026-08-25)**: confirmed via code read that `resumeListening()` (called after every voice turn) reuses the already-loaded `KeywordSpotter` instance — `kws` is only ever released in `onDestroy()`, not in the stop/resume path used between wake events. The ~14 MB ONNX encoder/joiner is loaded once per service lifetime, not once per wake — no fix needed here.
- **Telegram background polling** (not originally listed in this section, added by the Phase 13 audit): `TelegramService._pollUpdates` uses the Bot API's own long-polling pattern (30s server-side timeout per request, 1s gap between calls when idle) — this is the vendor-recommended pattern, not a tight loop, and only runs at all when the user has opted into Telegram remote control. No change needed.
